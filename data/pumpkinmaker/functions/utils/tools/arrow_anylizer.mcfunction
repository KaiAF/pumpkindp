...
execute as @e[type=arrow,nbt={item:{tag:{CustomTags:["BeeHoneyPot"]}}}] at @s positioned ^ ^ ^.1 unless block ~ ~ ~ air run function pumpkinmaker:utils/tools/func/create_honey_pot with storage pumpkindb:static.values HoneyPotTeam