...
execute if entity @s[tag=dev.mode,scores={honeyPotNames=0..}] as @e[tag=pumpkindb.honeypot.manager] run schedule function pumpkinmaker:utils/tools/func/disable_names 5s replace
execute if entity @s[tag=dev.mode,scores={honeyPotNames=0..}] as @e[tag=pumpkindb.honeypot.manager] run data modify entity @s CustomName set from entity @e[tag=pumpkindb.honeypot.text,limit=1,sort=nearest] CustomName
