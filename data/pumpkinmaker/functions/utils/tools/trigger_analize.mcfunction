...
execute if entity @s[tag=dev.mode,scores={honeyPotNames=0..}] as @e[tag=pumpkindb.honeypot.manager] at @s run schedule function pumpkinmaker:utils/tools/func/disable_names 5s replace
